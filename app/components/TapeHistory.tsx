...
export function TapeHistory({ assetId }: { assetId?: Hex }) {
  ...
  const load = useCallback(async (cancelled?: () => boolean) => {
    ...
    try {
      ...
      const end = Math.max(0, numericCount - page * PAGE_SIZE);
      const start = Math.max(0, end - PAGE_SIZE);
      const reads = Array.from({ length: end - start }, (_, offset) => {
  ...
  return (
    <section className="tape-history" aria-labelledby={assetId ? 'asset-history-title' : 'global-history-title'} data-reveal>
      <div className="tape-history-heading">
        <div><span className="mini-title">APPEND-ONLY AUDIT TRAIL</span><h2 id={assetId ? 'asset-history-title' : 'global-history-title'}>{assetId ? 'How this title reached today.' : 'Latest activity across the tape.'}</h2><p>Old entries remain intact when a slice changes. Newest entries come first; page back for older transitions.</p></div>
      ...
      {!loading && error == null && entries.length > 0 && <>
          ...
          {entries.map((entry) => {
            ...
            return <li key={entry.index} data-state={entry.state ?? 'unknown'}>
              ...
              <article className="tape-history-card">
                <div className="tape-history-event"><div><span className="tape-history-index">ENTRY {entry.index + 1}</span><h3>{eventVerb(entry.state)}</h3></div>{entry.state === 'due' ? <Badge tone="neutral" symbol="event">Opened</Badge> : entry.state ? <RecordStateBadge state={entry.state} /> : <Badge tone="neutral">Unknown</Badge>}</div>
                ...
                {definition && <p>{entry.state === 'due' ? 'The contract opened this payment window with its deadline fixed.' : definition.meaning}</p>}
        ...
        <Pagination label="Tape history pages" page={page} total={total} pageSize={PAGE_SIZE} onPageChange={setPage} noun="entries" />
      </>}
    </section>
  );
}
