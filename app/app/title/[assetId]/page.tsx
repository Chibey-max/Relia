...
export default function TitleOwnershipPage() {
  ...
  return (
    <main className="task-main ownership-main">
      ...
      {!loading && notFound && <DataEmptyState symbol="help" title="Title not found" body={validAssetId ? 'No listed title exists for this asset ID.' : 'This URL does not contain a valid 32-byte asset ID.'} action={<Button href="/title" variant="secondary">Find another title</Button>} />}
      ...
      {!loading && title && <>
        ...
        {stage !== 'idle' && <ol className="ownership-write-progress" aria-label="Title transaction progress" aria-live="polite">{WRITE_STAGES.map((item, index) => <li data-state={index < currentStage ? 'done' : index === currentStage ? 'active' : 'pending'} key={item.key}><i>{index < currentStage ? <MaterialIcon name="check" /> : index + 1}</i><span>{item.label}</span></li>)}</ol>}
